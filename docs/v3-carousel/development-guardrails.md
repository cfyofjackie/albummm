...
- 不为了填满空间而强裁切；极端比例优先完整展示。
  **唯一例外（产品确认）：** 毛边材质允许把照片边缘撕掉一块 —— 撕深按卡片短边算，
  主撕边 8.5%、其余三条 2.8%，硬上限 **12%**。除这条之外任何情况都不得裁切照片
...
- **渲染轴：边缘（毛边）。** 它不改尺寸、不改碰撞，只裁切轮廓 —— 但**同一套轮廓必须同时裁到
  相纸与照片本体**（`tornContours` 同时返回 `paper` 与 `image` 两组 polygon），否则只会「白纸被撕、
