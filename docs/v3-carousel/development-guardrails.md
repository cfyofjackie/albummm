...
**材质（边框 / 撕边 / 胶带）是独立于风格的额外选项，不是第四种风格。** 它可以和任意风格组合，
但必须通过 `carouselPlacement.js` 的 `FRAME_STYLES` 进入几何层 ——
**禁止只在 CSS 里加厚边框**：白边是内容与「外卡面相叠」之间的缓冲，厚度必须同时参与
`sizeFor / innerBox / 碰撞判定`，否则「照片内容区不得碰撞」的保证会失效。
边框值存在卡片上（`card.mat`），渲染层与判定层必须用同一份值，不得各自重算（见 `validation-log.md` §12）。
